...
1. 引入WAGEPAN.rda這個資料。
```{r}
load("C:\\Users\\ASUS\\Documents\\Part-II-Assignment\\WAGEPAN.rdata")
WAGEPAN
```
...
#nr與year變數皆為Factor，且勞動者有545個、年份有8年
```
...
library(ggplot2)
attach(WAGE_data)
ggplot(WAGE_data,aes(x=union,y=lwage,group=union)) + geom_boxplot()
#可以看到未加入公會的盒形圖全距較大、最大值較高、最小值較小、平均值較低；加入公會的盒形圖全距較小、四分位距也縮小、平均值則較高
```
...
```{r}
WAGE_data <- data.frame(WAGE_data)
WAGE_data$educ.high <- ifelse(WAGE_data$educ>=12,1,0)

WAGE_data.lwage.means  <- WAGE_data %>% group_by(union,educ.high) %>% summarise(mean(lwage))

WAGE_data.lwage.means
#結果顯示教育年數低於12年的薪資明顯較低，而且有加入公會的薪資都更高
```
...
```{r, message=FALSE, warning=FALSE}
mod1 <- lwage ~ union
mod2 <- lwage ~ union + educ.high
pool1<-plm(mod1, data=WAGE_data, model='pooling')
pool2<-plm(mod2, data=WAGE_data, model='pooling')
library(stargazer)
stargazer(pool1,pool2,type="text")
#在回歸模型中，教育程度確實顯著影響薪資，而且加入教育程度變數後，會降低公會影響薪資的係數
```

10. 分別計算個別勞動者在樣本期間的平均lwage與平均union，接著畫出兩者間的離散圖及在圖面上加上估計迴歸線。由兩者間的關係，說明勞動者的固定效果如何影響lwage與union兩個變數。
```{r}
WAGE_data.mean <- WAGE_data %>% group_by(nr) %>% summarise_each(funs(mean)) 
WAGE_data.mean
fe1 <- lm(mod1, data=WAGE_data)
fep1 <- plm(mod1, data=WAGE_data, model='within', effect='individual')
fe2 <- lm(mod2, data=WAGE_data)
fep2 <- plm(mod2, data=WAGE_data, model='within', effect='individual')
...
WAGE_data.mean%>%ggplot(aes(x=union,y=lwage)) + geom_point() + stat_function(fun=function(x){
  fe1$coefficients[1] + fe1$coefficients[2]*x
  },color="blue",linetype=2) + stat_function(fun=function(x){
  fe2$coefficients[1] + fe2$coefficients[2]*x
  },color="green",linetype=4)
#可以看到有加入教育程度的回歸線在未加入的回歸線下方
```
...
```{r, warning=FALSE}
fep1<-plm(mod1, data=WAGE_data, model='within', effect='individual')
rep1<-plm(mod1, data=WAGE_data, model='random')
stargazer(fep1,rep1,type="text") 
```

13. 請檢定固定效果是否與勞動者的工會狀態有關連，並說明適不適合使用隨機效果模型。
```{r}
phtest(fep1,rep1)
#p-value<0.05，表示固定效果與勞動者的工會狀態有關
```

14. 請檢定是否存在單純與個人特質有關的隨機效果項，並說明適不適合使用Pooled OLS。
```{r}
pwtest(pool1)
#p-value<0.05，存在個人特質的隨機效果項
```
