...
```{r}
#WAGE_data <- WAGE_data %>% group_by(nr) %>% mutate(mean(lwage))
#WAGE_data <- WAGE_data %>% group_by(nr) %>% mutate(mean(union))
#WAGE_data %>% group_by(nr) %>% ggplot(aes(x=mean(union) , y=mean(lwage))) + geom_point()
#model3 <- mean(lwage)~ mean(union)
#model3$coefficients
#ggplot(data=WAGE_data, aes(x=mean(union),y=mean(lwage)), color=nr) + geom_point() -> f1
#f1
#f1 + stat_function(fun=function(x){model3$coefficients[1]+model3$coefficients[2]*x}, color="black") -> f.ols
#f.ols
```
