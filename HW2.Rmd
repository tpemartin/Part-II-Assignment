...
首先請先執行以下程式，清空環境並引入Panel分析的套件plm。
 ...
 @@ -31,9 +34,9 @@ WAGE_data<-pdata.frame(WAGEPAN,c("nr","year"))
 ...
 @@ -55,8 +58,6 @@ levels(WAGE_data$year)
 ...
 @@ -73,7 +74,7 @@ WAGE_data %>% dplyr::group_by(union) %>% dplyr::summarise(mean(lwage))
 ...
 +### 圖中可以看出隨著教育年限上升，總體平均薪資也會跟著上升。 
  
  WAGE_data %>% dplyr::group_by(union,educ) %>% dplyr::summarise(mean(lwage)) -> WAGE.mean
  names(WAGE.mean)[3]<-"mlwage"
 @@ -83,7 +84,12 @@ WAGE.mean %>% ggplot(aes(x=educ,y=mlwage,color=union))+geom_point()
 ...
 +## 薪資會受到來自教育年限和工會狀態有相關性，平均來看，其他條件不變下，教育年限越高，平均薪資越高（教育提高了人力資本）；加入工會的平均薪資比沒加入高（工會作為一個勞方阻止，提高了工薪階層與資方的議價能力）。
  ...
  ### 模型估計
 @@ -92,34 +98,64 @@ $$lwage_{it}	=\beta_{0}+\beta_{1}union_{it}+\epsilon_{it}...(model1)$$
