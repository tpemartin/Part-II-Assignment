...
1. 引入WAGEPAN.rda這個資料。
```{r}
load("C:/Users/ssu/Desktop/Part-II-Assignment/Part-II-Assignment/WAGEPAN.rdata")
WAGEPAN
```
...
```{r}
year.order<-ordered(WAGE_data$year)
levels(year.order)
```
...
WAGE_data %>% ggplot(aes(x=WAGE_data$union,y=WAGE_data$lwage))+geom_boxplot()
## 從圖中可以明顯的看出，加入工會的人的工資分佈相對更為集中，且其最小值、中位數、上四分位數、下四分位數比沒加入工會的有明顯的提高，但最大值變小，看起來工會有降低高工資，提高低工資，使工資分佈更為集中的效果。
```
...
```{r}
WAGE_data %>% group_by(educ)%>% summarise(mean(lwage))-> t2
t2
```
...
```{r}
WAGE_data$educ.high<-ifelse(WAGE_data$educ>=12,1,0)

WAGE_data %>% group_by(educ.high,union) %>% summarise(mean(lwage))
## educ.high=0時，也就是教育年限小於12，此時不管工作狀態如何(union為TRUE or FALSE)， 皆會有較低的平均LOG薪資，而在union固定為FALSE下，教育程度高會有較多的平均LOG薪資
```
...
```{r, message=FALSE, warning=FALSE}
model_1<-lwage~union
model_2<-lwage~union+educ.high
pool1<-plm(model_1,WAGE_data,model = "pooling")
pool2<-plm(model_2,WAGE_data,model = "pooling")
summary(pool1)
summary(pool2)
...
WAGE_data %>% group_by(nr) %>% summarise_each(funs(mean))->id_mean
id1<-lm(model_1,data=id_mean)
id2<-lm(model_2,data=id_mean)
id_mean %>% ggplot(aes(x=union,y=lwage)) + geom_point()+stat_function(fun=function(x){id1$coefficients[1]+id1$coefficients[2]*x},color="black",linetype="solid") + stat_function(fun=function(x){id2$coefficients[1]+id2$coefficients[2]*x},color="black",linetype="dashed")
##教育這個固定效果，看起來會使得沒加入工會的薪資降低，但會使參加工會的效果提升
```
...
class(WAGE_data$educ.high)
class(WAGE_data$union)
##union為邏輯變數，所以不能demean，教育年限的離散度為0，因為教育年限不會隨時間改變，不會因為時間不同而教育年限有所變動。其他兩者有可能隨時間變動，如時間越久可能會有較高薪資。
```
...
```{r, warning=FALSE}
fixed_wage<-plm(model_1,data=WAGE_data, model ="within", effect ="individual")
random_wage<-plm(model_1,data = WAGE_data, model = "random")
stargazer(fixed_wage,random_wage,type = "text")

```

13. 請檢定固定效果是否與勞動者的工會狀態有關連，並說明適不適合使用隨機效果模型。
```{r}
phtest(fixed_wage,random_wage)
#p-value<5%，固定效果與工會狀態顯著有關聯，不適合使用隨機效果來估算
```

14. 請檢定是否存在單純與個人特質有關的隨機效果項，並說明適不適合使用Pooled OLS。
```{r}
pwtest(pool1)
#不適合，不能忽略群組效果與時間效果，不然估計會產生偏誤。
```
