...
1. 引入WAGEPAN.rda這個資料。
```{r}
load("C:/Users/asus/Deskktop/School/Graduate School TW/Quan/Part-II-Assignment/WAGEPAN.rdata")
```
...
從相形圖看來，有加入公會的人之lwage的平均相較於沒加入的人高，而且離散程度也較小
表示加入工會能確保有比較高的穩定薪資。
```
...
教育年限較高的人有較高的lwage。加入公會的人，平均教育年限比沒加入的高，所以有可能受到教育年限的影響。
```
...
WAGE_data %>% group_by(unionlogic, educ.high) %>%
  summarise(mean(lwage))
無論教育程度高低<有加入公會的lwage較高
```
...
#WAGE_data %>% group_by(nr) %>% ggplot(aes(x=mean(union) ,
y=mean(lwage))) + geom_point()
#model3 <- mean(lwage)~ mean(union)
#model3$coefficients
#ggplot(data=WAGE_data, aes(x=mean(union),y=mean(lwage)), color=nr) +
geom_point() -> f1
#f1
#f1 + stat_function(fun=function(x)
{model3$coefficients[1]+model3$coefficients[2]*x}, color="black") ->
f.ols
#f.ols
```
...
```{r}
WAGE_data$lwage_demean<-Within(WAGE_data$lwage,
effect=c("individual"))
WAGE_data$union_demean<-Within(WAGE_data$union,
effect=c("individual"))
...
educ和union在demean後的離散程度為零，可能是因為大部分員工得教育年限並不會隨時間變動
```
...
fel <-plm(model4, data=WAGE_data, model="within", effect="twoways")
rel <-plm(model4, data=WAGE_data, model="random")
summary(fel)
summary(rel)
```
...
pwtest(pool1)
pwteat(pool2)
兩個pooling model的BP test的結果皆為reject H0，所以迴歸適合使用隨機效果模型
```

14. 請檢定是否存在單純與個人特質有關的隨機效果項，並說明適不適合使用Pooled OLS。
```{r}
phtest(fel, rel)
Hausman test的檢驗結果是reject H0，所以此模型不適合使用pooled ols
```
