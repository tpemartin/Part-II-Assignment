...
1. 引入WAGEPAN.rda這個資料。
```{r}
load("C:/Users/user/Desktop/WAGEPAN.rdata")
```
...
#加入公會比沒加入公會的薪資高一點(在中位數`四分位距`大小值面都比較高)，但差異不大。
#有加入公會的薪資比較聚集，沒加入公會較分散。
```
...
#明顯得看出教育年限超過10年時(大約是有高中學歷的人)對平均薪資有正相關，但教育年限為3-8年(大概是國中畢業)比較沒有相關性。
...
#可看出教育年限也有參加工會薪資較高，教育年限較低也沒參加工會薪資較低。
#教育年限較高平均而言薪資就會比較高(正相關)，有參加工會薪資也會較高。
#可能原因我覺得是工會本身對企業來說本來就比較具有影響力，因為如果參加工會老闆可能比較不敢壓榨勞工。如果壓榨勞工，勞工可能會集體罷工。所以參加工會新資可能會比較高。至於教育年限高薪資較高就比較直覺。
```
...
#拒絕虛無假設，接受對立假設。
#p-value小於0.05，所以勞工與工會有關聯，不適合隨機效果模型
```
...
pwtest(pool1)
#拒絕虛無假設，接受對立假設。
#p-value小於0.05，所以存在單純與個人特質有關的隨機效果項，不適合使用Pooled OLS
