...
```
#nr是因子變數，且有54個， year 也是因子型變數，有8年
...
```
#從圖中可以明顯的看出，加入工會的人工資分佈相對沒加入工會來說更窄，更為集中在中位數附近（四分位距變窄），且其最小值、中位數、上四分位數、下四分位數比沒加入工會的有明顯的提高，但最大值變小，看起來工會有降低高工資，提高低工資，並使工資分佈更為集中的效果。
...
```{r}
WAGE_data %>% ggplot(aes(x=educ,y=lwage,color=union))+geom_point()

```
#可以看到總體平均薪資隨著教育的上升有個上升的趨勢，但可發現似乎教育程度越高，薪資變異數越大。
...
```
# 薪資會受到來自教育年限和工會狀態有相關性，平均來看，其他條件不變下，教育年限越高，平均薪資越高（教育提高了人力資本）；加入工會的平均薪資比沒加入高（工會作為一個勞方，提高了勞方與資方的議價能力）。
### 模型估計
9. 考慮兩個迴歸模型：
...
```
# 教育年限的離散度為0，因為教育年限通常在工作后就固定了，他只會隨個體不同而不同，但不會隨時間改變，而加入工會、薪資會隨時間變動。
12. 考慮如下的模型
