...
```{r}
library(dplyr)
WAGE_data.lwage.means  <- WAGE_data %>% group_by(union) %>% summarise_each(col=lwage,funs(mean))
...
```{r}
mod1 <- lwage~union
reg1 <- lm(mod1,data=WAGE_data)
summary(reg1)
mod2 <- lwage~union+educ
reg2 <- lm(mod2,data=WAGE_data)
summary(reg2)
#教育年限確實會影響取對數的實質薪資
```
...
WAGE_data.lwage.means  <- WAGE_data %>% group_by(union,educ.high) %>% summarise_each(col=lwage,funs(mean))
...
```{r}
fe1<-plm(mod1, data=WAGE_data, model='within', effect='individual')
fe2<-plm(mod2, data=WAGE_data, model='within', effect='individual')
```


11. 對lwage, union及educ.high進行組內demean，並觀察demean後的變數離散度，那一個變數的離散度為零？為什麼？
```{r}
fatality$lwage_demean<-Within(fatality$lwage,effect=c('individual'))
fatality$union_demean<-Within(fatality$union,effect=c('individual'))
fatality$educ.high_demean<-Within(fatality$educ.high,effect=c('individual'))
```
...
```{r, warning=FALSE}
fe1_two<-plm(mod1, data=WAGE_data, model='within', effect='twoways')
re1<-plm(mod1, data=WAGE_data, model='random')
stargazer(fe1_two,re1,type="text")
```
