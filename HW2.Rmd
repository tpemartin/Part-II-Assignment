...
1. 引入WAGEPAN.rda這個資料。
```{r}
load("C:\\Users\\菲比\\Documents\\Part-II-Assignment\\WAGEPAN.rdata")
WAGEPAN
```
...
### 從圖中可以看出，加入工會的工資分佈相對更窄，更為集中在中位數附近，且其最小值、中位數、上四分位數、下四分位數比沒加入工會的有明顯的提高，但最大值變小，工會有降低最高工資以及提高最低低工資的效果。
```
...
## 薪資會受到教育年限和工會狀態而有相關性，平均而言，其他條件不變下，教育年限愈高，平均薪資愈高（教育提高了人力資本）；加入工會的平均薪資比沒加入高（工會提高了工薪階層與資方的議價能力）。
```
...
## 教育年限是一個重要的解釋變數，它會影響薪資也會影響一個人是否加入工會的決定。加入工會的工資雖然會提高，但會更集中在平均值附近，接受高等教育的人對自己有更大的期望，因此會選擇不加入工會以謀求更高的薪資。
```
...
# 從p-value看出檢定統計量顯著異於零,表示從在個人特質的隨機效果項,不適合使用Pooled OLS。
```
