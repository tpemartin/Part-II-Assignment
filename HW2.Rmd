...
1. 引入WAGEPAN.rda這個資料。
```{r}
load("C:\\Users\\YoZe\\Documents\\Part-II-Assignment\\WAGEPAN.rdata")
...
Ans:有加入工會比沒加入工會的離散程度小，所以有加入工會對薪資來說會較穩定。
```{r}
...
```{r}
WAGE_data %>% dplyr::group_by(union,educ) %>% dplyr::summarise(mean(lwage)) -> ueWAGE
ueWAGE
names(ueWAGE)[3]<-"mlwage"
ueWAGE %>% ggplot(aes(x=educ,y=mlwage,alpha=union))+geom_point()
```
...
Ans:教育程度越高的人，加入工會後能有更高的薪資，但教育程度高的人，薪資的漲幅卻比較低(不多)，可能是因為薪資上漲幅度本來就會遞減所造成的。
```{r, message=FALSE, warning=FALSE}
model1 <- lwage~union
...
Ans:這裡教育年限不會隨著時間改變，因為已經固定了，不像薪資會隨著時間而改變。
```{r}
...
Ans:Hausman test的p值小於5%，固定效果與工會狀態有關聯，不適合使用隨機效果模型。
```{r}
phtest(FE,RE)
```
...
Ans:可看出其P value很小，故不適合使用Pooled OLS。
```{r}
pool1
pwtest(pool1)
```
