...
1. 引入WAGEPAN.rda這個資料。
```{r}
load("C:/Users/jason/Desktop/part-II-assignment/WAGEPAN.rdata")
WAGEPAN
```
...
```
#1.有加入公會的lwage離散程度較小，沒有加工會的薪資離散程度較大，可以猜測為加入工會後，工作薪資能受到一定的保障。
#2.加入工會後的平均薪資雖然高於未加入工會的均薪資，但能觀察到加入公會的薪資大多低於平均，或許可以從觀察加入公會的人的教育水準或工作性質得到解答。


6. 請計算比較不同工會狀況的平均lwage差異[hint: 在作業1中，我們學到使用dplyr::group_by()與dplyr::summarise()來計算不同群組的組內特徵。]
```{r}
library(dplyr)
out1 <- summarise(group_by(WAGE_data,union),mean(lwage))
out1
```

 7. 我們擔心“單純比較不同工會狀態的平均薪資，反應出來的可能不會是單純的工會效果。會不會還包含教育年限的影響？” 請在不做迴歸估計下，單純觀察資料來判斷以上的擔心是否成立。
```{r}
out2 <- summarise(group_by(WAGE_data,educ),mean(lwage))
out2
mean.lwage <- out2$`mean(lwage)`
ggplot(out2,aes(x = educ, y = mean.lwage))+geom_point()
```
#從散佈圖可以看到當教育年限越高平均lwage也會越來越高，因此可以推斷教育年限會影響平均薪資。
...
```{r}
educ.high<-ifelse(WAGE_data$educ >= 12, 1, 0)
WAGE_data <- cbind(WAGE_data,educ.high)
WAGE_data
out3 <- summarise(group_by(WAGE_data,educ.high,union),mean(lwage))
out3
```
...
```
#因model1將教育年限放入殘差項，因此兩個model的Beta1係數會稍微不同。
...
library(psych)
describe(WAGE_data$lwage)
describe(WAGE_data$lwage_demean)
```
#從結果可以得知lwage demean後的離散程度不為零，但其離散程度有比demean前的離散程度小。
#union 與 educ.high 不知如何 demean

12. 考慮如下的模型
$$lwage_{it}=\beta_{1}union_{it}+c_{i}+\epsilon_{it}$$
請分別從固定效果（其估計式有時稱為within estimator）及隨機效果來估算$\beta_1$。
```{r, warning=FALSE}
out5 <- plm(model1,WAGE_data,model = 'within', effect = "individual")
out6 <- plm(model1,WAGE_data,model = "random", effect = "individual")
stargazer(out5,out6,type = 'text')
```
