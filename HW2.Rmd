...
```{r, message=FALSE, warning=FALSE}
model1<- lwage~union
model1 %>% plm(data=wage_data,model="pooling")
model2<- lwage~union+educ.high
model2 %>% plm(data=wage_data,model="pooling")
library(stargazer)
...
nrdata
nrdata %>% ggplot(aes(x=`mean(union)`,y=`mean(lwage)`))+geom_point()+geom_smooth(moedel=lm)->f1
f1
```
