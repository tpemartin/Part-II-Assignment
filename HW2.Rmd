...
getwd() get working directory 查工作路徑
setwd("中間放路徑") 設新的工作路徑
1. 引入WAGEPAN.rda這個資料。
```{r}
...
wage_data %>% group_by(educ) %>% summarise(mean(lwage),sd(lwage))
```
加入工會的薪水比沒加工會多18%
...
meanlwage %>% ggplot(aes(x=union,y=`mean(lwage)`,color=educ))+geom_point()
meanlwage %>% ggplot(aes(x=educ,y=`mean(lwage)`,color=union))+geom_point()
wage_data %>% group_by(educ) %>% summarise(munion=mean(union)) -> data2
data2 %>% ggplot(aes(x=educ,y=munion))+geom_point()
```
The worry is correct, higher education would have higher wage. We have to control the effect of education.A3假設，要寫到對Y的影響和對X的影響
wage_data %>% group_by(educ,union) %>% summarise(mlwage=mean(lwage)) -> meanlwage，在summarise中直接加入要的變數名稱即可
data2 %>% ggplot(aes(x=educ,y=munion))+geom_point():
educ and munion has positive correlation, however, educ is high enough, the percentage that join a union is decreasing.(y axis is the probability that who join the union)
...
At the same educ.educ, the wage is only 16% higher than those who doesn't join the union.
model1 overestimate.
...
wage_data %>% ggplot(aes(x=educ.high_demean,y=lwage_demean,color=union_demean))+geom_point()
```
...
```
fe: wage increasing 7.5%(C_i and union are related); re: wage increasing 10.2%(assume C_i and union are not related). That is, fixed effect(C_i) is related to union.
