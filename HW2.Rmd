...
1. 引入WAGEPAN.rda這個資料。
```{r}
load("D:/DownLoads/WAGEPAN.rdata")
WAGEPAN
```
...
#從圖中可知，有加入工會的人其平均薪資分佈較集中，而上四分位數、中位數、下四分位數都較大。
```
...
#教育年限中6年、8年、10年、11年、12年、13年、14年，有參加工會的人平均薪資確實比較高。
```
...
#平均薪資確實會受到工會狀態和教育年限所影響，當人們受到愈多的教育(人力資本提升)可以拿到更多的平均薪資，且因為有工會的存在，可以幫勞工爭取更多的平均薪資，不讓資方輕易剝削，即工會提高了勞工的議價能力。
```
...
library(stargazer)
reg1 <- lwage~ union
reg2 <- lwage~ union + educ.high

pols1 <- plm(reg1,data=WAGE_data,model="pooling")
pols1
pols2 <- plm(reg2,data=WAGE_data,model="pooling")
pols2
stargazer(pols1,pols2,type="text")
#考慮了高等教育年限後，有參加工會的人的平均薪資會少增加1.4%，意思是平均薪資不只受到工會狀態所決定，是否有受高等教育也會影響平均薪資。
#如同前面的箱型圖所示，
```
...
WAGE_data2
pols1$coefficients
WAGE_data2 %>% ggplot(aes(y=`mean(lwage)`,x=`mean(union)`))+geom_point()+stat_function(fun=function(x){pols1$coefficients[1]+pols1$coefficients[2]*x},color="black",linetype="dashed")->wage.ols.pols1
wage.ols.pols1
#在八年之內，平均參加工會程度愈高者，其平均薪資也會愈高。
```
...
#邏輯型變數無法demean，因為用一個True/False去減平均值是很怪的一件事。
...
```{r, warning=FALSE}
fixid_wage<-plm(reg1,data=WAGE_data, model ="within", effect ="individual")
r_wage<-plm(reg1,data = WAGE_data, model = "random")
...
#Hausman-test的p-value<5%，固定效果與工會狀態會有關係，不適合使用隨機效果模型。
```
...
pols1
pwtest(pols1)
#存在與個人特質有關的隨機效果項，不適合使用Pooled OLS。
```
